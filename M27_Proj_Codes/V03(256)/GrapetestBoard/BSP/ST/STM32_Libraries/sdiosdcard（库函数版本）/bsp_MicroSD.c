...
SD_Error SD_PowerON(void)
{
	...
	SDIO_InitStructure.SDIO_ClockDiv = SDIO_INIT_CLK_DIV;	       //初始化速度400kHz
	...
	for(count = 0;count < 74;count++)
	{
		...
		if(errorstatus != SD_OK)	return errorstatus;	//操作失败
		else	break;	//操作成功了退出 ：SDIO_FLAG_CMDSENT  ==1 
	}
	count = 0;
	...
	/*!< CMD55 */
	SDIO_CmdInitStructure.SDIO_Argument = 0x00;
	SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_APP_CMD;  //下次发送应用命令
	SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
	SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
	SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
	SDIO_SendCommand(&SDIO_CmdInitStructure);
...
{   //校验正确
	...
	SDIO->ICR = SDIO_STATIC_FLAGS; //清除所有静态标志
